...
const ThreadEmojis = ({ toggleModal }) => {
  ...
  const userReacted = (key) => {
    ...
    const comment = message.comments.find(
      (comment) => comment._id === currenctCommentId
    );

    return comment.reactions.users_who_reacted.some(
      (reaction) =>
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-full pb-24 flex justify-between">
          {Object.entries(reactionsDicc).map(([key, value]) => (
            <button
              key={key}
              onClick={(e) =>
                handleEmojiComment(e, key, currenctCommentId, selectedId, true)
              }
