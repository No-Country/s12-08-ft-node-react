...
const CardSubscription = ({ data, isSubs }) => {
  ...
  return (
    <div onClick={handleNavigate} className="card text-neutral-content relative rounded-[20px] bg-slate-50">
      <div className="card-body items-center text-center b" style={backgroundChat}>
        {/* Menu ... */}
        <div className="dropdown dropdown-end right-0 top-0 absolute">
          <label tabIndex={0} className="btn btn-ghost btn-circle avatar" onClick={(e) => e.stopPropagation()}>
            <Option />
          </label>
          <ul
            tabIndex={0}
